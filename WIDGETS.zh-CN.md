...
- `jsonui` ??????? `iconSizeDP` ? `iconPolicy`?DPI ????? `Window.ReloadResources(...)` ???
- `jsonui.MountWindow(...)` / `WindowHost.ReplaceWindow(...)` / `WindowHost.Detach()` ???????????? API?
- `widgets.Modal` ?? JSON `type: "modal"`??? backdrop ???dismiss ??? Direct2D-only ????????

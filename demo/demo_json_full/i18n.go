...
func parseListItems(raw any) []widgets.ListItem {
	nodes, ok := raw.([]any)
	if !ok {
		return nil
	}
...
func (l *demoLang) fileFilters(locale string, path string, fallback []sysapi.FileFilter) []sysapi.FileFilter {
	raw, ok := l.value(locale, path)
	if !ok {
		return append([]sysapi.FileFilter(nil), fallback...)
	}
	nodes, ok := raw.([]any)
	if !ok {
		return append([]sysapi.FileFilter(nil), fallback...)
	}
	filters := make([]sysapi.FileFilter, 0, len(nodes))
	for _, node := range nodes {
		entry, ok := node.(map[string]any)
		if !ok {
			continue
		...
		filters = append(filters, sysapi.FileFilter{
			Name:    name,
			Pattern: pattern,
		})
	}

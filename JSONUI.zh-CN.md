...
- `label` ??????? `multiline` ? `wordWrap`??????????????? label ????????
- ?????????????? `iconSizeDP` ? `iconPolicy: "auto" | "fixed"`?`fixed` ?? DPI ??????????????????
- JSON ?? `type: "modal"`??? `backdrop.color`?`backdrop.opacity`?`backdrop.blur`?`backdrop.dismissOnClick` ? `onDismiss`?
- `backdrop.blur` ?? Direct2D ????????????GDI ????????????
- ?????????? `jsonui.MountWindow(scene, win)` ??? `WindowHost`???? `ReplaceWindow(...)` / `Detach()`?
- ?? hot reload ???????????? + ????????????????????????? UI ?????

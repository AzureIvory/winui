...
// ListBox 表示单选列表控件�?type ListBox struct {
	widgetBase
	items    []ListItem
	selected int
	hover    int
...
// NewListBox 创建一个新的列表框�?func NewListBox(id string) *ListBox {
	return &ListBox{
...
// SetBounds 更新列表框的边界�?func (l *ListBox) SetBounds(rect Rect) {
	l.runOnUI(func() {
		l.widgetBase.setBounds(l, rect)
	})
}

// SetVisible 更新列表框的可见状态�?func (l *ListBox) SetVisible(visible bool) {
	l.runOnUI(func() {
		l.widgetBase.setVisible(l, visible)
	})
}

// SetEnabled 更新列表框的可用状态�?func (l *ListBox) SetEnabled(enabled bool) {
	l.runOnUI(func() {
		l.widgetBase.setEnabled(l, enabled)
	})
}

// SetItems 更新列表框的项目集合�?func (l *ListBox) SetItems(items []ListItem) {
	l.runOnUI(func() {
...
// Items 返回列表框所管理项目的副本�?func (l *ListBox) Items() []ListItem {
	return cloneItems(l.items)
}

// SetSelected 更新列表框的当前选择�?func (l *ListBox) SetSelected(index int) {
	l.runOnUI(func() {
		l.selectIndex(index, false)
	})
}

// SelectedIndex 返回列表框当前选中的索引�?func (l *ListBox) SelectedIndex() int {
	return l.selected
}

// SelectedItem 返回列表框当前选中的项目�?func (l *ListBox) SelectedItem() (ListItem, bool) {
...
// SetStyle 更新列表框的样式覆盖�?func (l *ListBox) SetStyle(style ListStyle) {
	l.runOnUI(func() {
		l.Style = style
		l.invalidate(l)
	})
...
// SetOnChange 注册列表框的变更回调�?func (l *ListBox) SetOnChange(fn func(int, ListItem)) {
	l.runOnUI(func() {
		l.OnChange = fn
	})
}

// OnEvent 处理输入事件或生命周期事件�?func (l *ListBox) OnEvent(evt Event) bool {
	switch evt.Type {
	case EventMouseMove:
...
// Paint 使用给定的绘制上下文完成绘制�?func (l *ListBox) Paint(ctx *PaintCtx) {
...
// acceptsFocus 返回控件是否可接收键盘焦点�?func (l *ListBox) acceptsFocus() bool {
	return true
}

// cursor 返回悬停控件时应使用的光标�?func (l *ListBox) cursor() CursorID {
	if !l.Enabled() {
...
// resolveStyle 解析列表框的最终样式�?func (l *ListBox) resolveStyle(ctx *PaintCtx) ListStyle {
...
// handleKey 处理列表框的按键事件�?func (l *ListBox) handleKey(key core.KeyEvent) bool {
...
// selectRelative 按给定步长移动当前选择，并跳过禁用项�?func (l *ListBox) selectRelative(step int) {
	index := l.selected
	if index < 0 {
		if step >= 0 {
			index = -1
...
// selectIndex 将当前选择设置为指定项索引�?func (l *ListBox) selectIndex(index int, notify bool) {
...
// indexAt 返回指定位置对应的项索引�?func (l *ListBox) indexAt(point core.Point) int {
	rect := l.Bounds()
...
// rowRect 返回列表行的绘制矩形�?func (l *ListBox) rowRect(index int, ctx *PaintCtx, style ListStyle) Rect {
...
// dp 按应用当�?DPI 缩放设备无关值�?func (l *ListBox) dp(value int32) int32 {
...
// mergeListStyle 将列表框样式覆盖合并到基础样式上�?func mergeListStyle(base, override ListStyle) ListStyle {
